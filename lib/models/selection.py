...
class MultiHeadSelection(nn.Module):
    ...
    def forward(self, sample, is_train: bool) -> Dict[str, torch.Tensor]:
        ...
        tokens = sample.tokens_id.cuda(self.gpu) if torch.cuda.is_available() else sample.tokens_id
        selection_gold = sample.selection_id.cuda(self.gpu) if torch.cuda.is_available() else sample.selection_id
        bio_gold = sample.bio_id.cuda(self.gpu) if torch.cuda.is_available() else sample.bio_id
        ...
        if is_train:
            ...
            bio_gold = torch.tensor(temp_tag).cuda(self.gpu) if torch.cuda.is_available() else torch.tensor(temp_tag)
